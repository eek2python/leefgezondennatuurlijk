...
class KoolstofstalenKoekenpannenIntegrationTests(TestCase):
    def test_category_page_renders_seo_content_and_size_selection(self):
        response = self.client.get(
            "/koolstofstalen-koekenpannen/", {"size": "24"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response,
    ...
    def test_menu_homepage_and_related_categories_link_to_category(self):
        category_url = "/koolstofstalen-koekenpannen/"
    ...
    def test_product_detail_returns_to_carbon_steel_category(self):
        response = self.client.get("/product/debuyer-mineral-b-28/")
        self.assertEqual(response.status_code, 200)
...
class ProductLinkResolverTests(TestCase):
    ...
    PARTNER_BOL = "https://partner.bol.com/click?p=123&t=product&s=456"
    ...
    def test_partner_bol_affiliate_url_gets_bol_label(self):
        link = resolve_product_link({"affiliate_url": self.PARTNER_BOL})
        self.assertEqual(link.label, "Bekijk prijs en reviews bij Bol →")
        self.assertEqual(link.link_type, "affiliate")
        self.assertIn("sponsored", link.rel)

    def test_partner_bol_retailer_url_gets_bol_label(self):
        link = resolve_product_link({"retailer_url": self.PARTNER_BOL})
        self.assertEqual(link.label, "Bekijk prijs en reviews bij Bol →")
        self.assertEqual(link.link_type, "retailer")
        self.assertNotIn("sponsored", link.rel)

    def test_partner_bol_official_url_keeps_specs_label(self):
        # official_url krijgt nooit de Bol-knoptekst, ook niet bij partner.bol.com.
        link = resolve_product_link({"official_url": self.PARTNER_BOL})
        self.assertIn("productspecificaties", link.label.lower())
    ...
    def test_is_bol_nl_url_detection(self):
        ...
        # partner.bol.com: affiliate-tracking-domein, geen /nl/-padeis.
        self.assertTrue(is_bol_nl_url("https://partner.bol.com/click?p=1&t=product"))
        self.assertTrue(is_bol_nl_url("https://partner.bol.com/"))
        # Lookalike-domeinen mogen niet matchen.
        self.assertFalse(is_bol_nl_url("https://partner.bol.com.evil.example.com/"))
        self.assertFalse(is_bol_nl_url("https://bol.com.example.com/nl/x"))
